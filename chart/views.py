...
from . db import StockDataGetter, CompanyDataGetter, StockDBBuilder
from django.http import HttpResponse
# Create your views here.

def dbmaker(request) :
	stock_id = request.GET('stock_id')
	com_name = request.GET('name')
	StockDBBuilder().addNewCompany(stock_id, com_name)
	return HttpResponse('')

def dbupdater(request) :
	StockDBBuilder().updateOnePage(rows=2)
	return HttpResponse('')

def showChartPage(request) :
	context = {}
	question = request.GET['question']
	stock_symbol = CompanyDataGetter().recogCompanyName(question)
	context['symbol'] = stock_symbol

	if stock_symbol is None :
		context['data'] = _getDemoData()
	else :
		context['data'] = StockDataGetter().getStockData(stock_symbol)
